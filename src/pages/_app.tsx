...
export default function MyApp({ Component, pageProps }: ExtendedAppProps) {
  ...
  // --- Preserve current page on sign-in ---
  useEffect(() => {
    if (user && isOpen) {
      close()
      // Get stored redirect path or stay on current page
      const storedPath = typeof window !== 'undefined' ? sessionStorage.getItem('authRedirectPath') : null
      if (storedPath) {
        sessionStorage.removeItem('authRedirectPath')
        router.push(storedPath)
      }
      // If no stored path, stay on current page (don't redirect)
    }
  }, [user, isOpen, close, router])

  // Store current path when auth modal opens
  useEffect(() => {
    if (isOpen && !user) {
      // Store current path for redirect after sign-in
      if (typeof window !== 'undefined') {
        sessionStorage.setItem('authRedirectPath', router.asPath)
      }
    }
  }, [isOpen, user, router.asPath])
